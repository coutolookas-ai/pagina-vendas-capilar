...
export default function SalesPage() {
  ...
  // Sistema AVANÇADO de supressão de erros - Intercepta ANTES do sistema de logging
  useEffect(() => {
    // 1. Interceptar console.error e console.warn
    ...
    console.error = (...args: any[]) => {
      ...
      if (
        message.includes('HlsVideo') || 
        message.includes('segment') ||
        message.includes('sentry.io') ||
        message.includes('converteai.net') ||
        message.includes('.ts') ||
    ...
    // 2. Interceptar erros globais do window
    const handleError = (event: ErrorEvent) => {
      if (
        event.message?.includes('HlsVideo') ||
        event.message?.includes('segment') ||
        event.message?.includes('converteai.net') ||
    ...
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      ...
      if (
        reason.includes('HlsVideo') ||
        reason.includes('segment') ||
        reason.includes('converteai.net') ||
    ...
    // 4. Interceptar XMLHttpRequest globalmente
    ...
    XMLHttpRequest.prototype.open = function(method: string, url: string | URL, ...args: any[]) {
      ...
      this._url = urlString
      // @ts-ignore
      this._shouldSuppress = urlString.includes('converteai.net') || urlString.includes('sentry.io')
  ...
  // Carregar script do Smartplayer com retry
  useEffect(() => {
      ...
      try {
        ...
        script.onerror = () => {
          if (retryCount < maxRetries) {
            retryCount++
            setTimeout(loadScript, 2000) // Retry após 2 segundos
          }
        }
